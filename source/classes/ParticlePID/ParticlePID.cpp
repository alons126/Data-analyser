...
void ParticlePID::FDNeutralParticle(vector <region_part_ptr> allParticles, vector<int> &ID_Neutrons_FD, vector<int> &ID_Photons_FD) {
    for (int i = 0; i < allParticles.size(); i++) {
        ...
        if ((allParticles[i]->par()->getCharge() == 0) && (allParticles[i]->getRegion() == FD) &&
            (ParticlePDG != 0)) { // If particle is neutral and in the FD
            ...
            if ((ParticlePDG == 2112) || (ParticlePDG == 22)) {
                if (ParticleInPCAL) {
                    ...
                    if (ParticleInECIN || ParticleInECOUT) {
                        bool NeutronPassVetoTest = NeutronECAL_Cut_Veto(allParticles, electrons, beamE, i, Neutron_veto_cut.GetLowerCutConst());
