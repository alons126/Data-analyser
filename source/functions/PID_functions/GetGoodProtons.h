...
vector<int> GetGoodProtons(vector<region_part_ptr> &protons, const vector<int> &Protons_ind,
    ...
    for (int i = 0; i < Protons_ind.size(); i++) {
        ...
        for (int j = i + 1; j < Protons_ind.size(); j++) {
            if ((protons[Protons_ind.at(i)]->getRegion() == CD) && (protons[Protons_ind.at(j)]->getRegion() == CD)) {
            ...
            } else if (((protons[Protons_ind.at(i)]->getRegion() == FD) && (protons[Protons_ind.at(j)]->getRegion() == CD)) ||
                       ((protons[Protons_ind.at(i)]->getRegion() == CD) && (protons[Protons_ind.at(j)]->getRegion() == FD))) {
                double Theta_p_i = protons[Protons_ind.at(i)]->getTheta() * 180.0 / pi, Phi_p_i = protons[Protons_ind.at(i)]->getPhi() * 180.0 / pi;
                double Theta_p_j = protons[Protons_ind.at(j)]->getTheta() * 180.0 / pi, Phi_p_j = protons[Protons_ind.at(j)]->getPhi() * 180.0 / pi;
        ...
        if (GoodProt) { GoodProtons.push_back(Protons_ind.at(i)); }
    }
