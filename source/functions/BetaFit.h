...
//    deltaPParam->AddText(("#delta#beta = " + to_string_with_precision(FitStd, 8)).c_str());
    ...
    cout << "Constan_Max = " << FitStd / deltaPRel_UncertaintyU << "\n";
    ...
    cout << "Constan_Max = " << FitStd / deltaPRel_UncertaintyL << "\n";
    ...
    string Rel_deltaPTitle = BetaParticle + " relative uncertainty #deltaP_{" + BetaParticleShort + "}/P_" + " (" + BetaPlot.GetFinalState() + ")";
    ...
    Rel_deltaP->GetYaxis()->SetTitle(("#frac{#deltaP_{" + BetaParticleShort + "}}{P} = #frac{1}{(1 - #beta^{2})#beta}").c_str());
...
//    string Wfunc = to_string(FitStd) + "/ ( (1 - x*x) * x )";
    ...
    W_Min->SetLineStyle(3);
    W_Min->SetLineColor(kBlack);
    W_Min->SetLineWidth(2);
    W_Min->Draw("same");
    Histogram_list->Add(W_Min);

    TLine *Beta_Max_cut = new TLine(Beta_Max, 0.04, Beta_Max, -0.12);
//    TLine *Beta_Max_cut = new TLine(Beta_Max, gPad->GetFrame()->GetY1() - 0.126, Beta_Max, gPad->GetFrame()->GetY2() + 0.5);
    ...
    TLine *Beta_Min_cut = new TLine(Beta_Min, 0.04, Beta_Min, -0.12);
