...
vector<int> GetFDNeutrons(vector<region_part_ptr> &allParticles, DSCuts &Momentum_cuts, bool apply_chi2_cuts_1e_cut) {
    ...
    for (int i = 0; i < allParticles.size(); i++) {
        ...
        if ((allParticles[i]->getRegion() == FD) && ((ParticlePDG == 2112) || (ParticlePDG == 22))) { // if neutron/photon is in the FD
            ...
            if (!inPCAL && (inECIN || inECOUT)) { // if the criteria for a particle to be a neutron is upheld
                ...
                double Momentum = GetFDNeutronP(allParticles[i], apply_chi2_cuts_1e_cut);
