...
//                                    "/C12x4_simulation_G18_Q204_6GeV_plots.root";
    ...
    const string AMapsFolderSim = "Event_Selection_Sim/06_AMaps";
    system(("mkdir -p " + AMapsFolderSim).c_str());

    const string AMapsFolderMapsSim = "Event_Selection_Sim/06_AMaps/01_Maps";
    system(("mkdir -p " + AMapsFolderMapsSim).c_str());

    const string AMapsFolderMomentumPlotsSim = "Event_Selection_Sim/06_AMaps/02_Momentum_Plots";
    system(("mkdir -p " + AMapsFolderMomentumPlotsSim).c_str());

    const string EfficiencyFolderSim = "Event_Selection_Sim/07_Efficiency";
    system(("mkdir -p " + EfficiencyFolderSim).c_str());

    const string EfficiencyRegFolderSim = "Event_Selection_Sim/07_Efficiency/01_No_FC";
    system(("mkdir -p " + EfficiencyRegFolderSim).c_str());

    const string EfficiencyWithFCFolderSim = "Event_Selection_Sim/07_Efficiency/02_wFC";
